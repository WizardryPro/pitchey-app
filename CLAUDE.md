...
- **Orchestrator agent — AIOps (pillar 3)**: Long-running agent consuming Sentry issue webhooks + Axiom alert routes; bounded remediations via Cloudflare + Sentry MCP (rollback, restart Durable Object, flush Hyperdrive cache). **Hard prerequisites (both required)**: (a) catch-swallow sweep complete — *untagged* `.catch(() => …)` < 30 in `src/` excluding `worker-integrated.ts` (telemetry sites tagged `// fire-and-forget` allowed); (b) Phase C.1 rollback drill verified end-to-end against staging — `wrangler rollback` is a bounded action, untested rollback is worse than no rollback. Roadmap Phase D.4. See `docs/strategy-five-pillars-2026-05-02.md`.
