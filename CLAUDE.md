...
- **Auth**: Custom sessions only on the backend (Better Auth backend rip merged 2026-05-04 per issue #19, path a). Legacy `users`/`sessions` tables, `pitchey-session` UUID cookie, custom login/logout/refresh handlers inlined in `src/worker-integrated.ts`. The session-store helper lives at `src/auth/session-store.ts` (4 raw-SQL methods used by the inline login paths). The auth-adapter at `src/auth/auth-adapter.ts` validates session cookies + JWT bearer tokens for `/api/users/profile` + `/api/users/settings`. WebSocket auth uses `src/auth/legacy-session-handler.ts`. **DB connections route through Hyperdrive** when bound (`env.HYPERDRIVE?.connectionString || env.DATABASE_URL` fallback in both `session-store.ts` and `legacy-session-handler.ts`); resource `dec8d1ab675649c8b69d88cb4c160704` (`pitchey-db-real`) has caching explicitly disabled — query result caching here would let revoked sessions keep authenticating until cache TTL expires. Rotation gotcha: Hyperdrive holds its own copy of the Neon credential, so rotation must update BOTH the Worker secret AND `wrangler hyperdrive update --connection-string=...` (codified in wrangler.toml comment). **Frontend BA scope is separate and unfinished** — `frontend/package.json` still declares `better-auth: ^1.4.10`; `frontend/src/lib/better-auth-client.tsx` invokes `createAuthClient` at module load but the resulting `authClient` is never consumed for HTTP (live frontend auth uses plain fetch wrappers via `betterAuthStore.ts`). Frontend rip tracked separately in #105 (post-Stripe). Phase 1.5b (drop empty BA tables `user`, `session`, `account`, `verification`) is gated and not yet scheduled. Why path (a) over (b): the 2026-05-02 verdict picked migration to BA, but re-reading the live code showed BA was never actually authenticating anyone — `dbAdapter` was a misnamed raw-SQL shim, BA library calls were guarded fallbacks behind legacy lookups. Same shape as the #20 pattern: reasoning from doc framing rather than runtime. Migration would have meant *building* a BA flow from scratch, not migrating between two working systems.
...
- Raw SQL only (no ORM)
- Sessions live in the legacy `sessions` table; cookie is `pitchey-session` (UUID). JWT bearer tokens are accepted as a fallback in `auth-adapter.ts:validateAuth`. Backend BA rip merged 2026-05-04 per issue #19. Frontend BA library still bundled but not consumed for HTTP — separate rip tracked in #105 (post-Stripe).
...
- **Orchestrator agent — AIOps (pillar 3)**: Long-running agent consuming Sentry issue webhooks + Axiom alert routes; bounded remediations via Cloudflare + Sentry MCP (rollback, restart Durable Object, flush Hyperdrive cache). **Hard prerequisites**: (a) catch-swallow sweep complete — **MET 2026-05-08** (all 11 PRs merged, untagged residue across `src/` = 0); (b) Phase C.1 rollback drill verified end-to-end against staging — **STILL CLOSED** (Phase C, post-Stripe per locked sequencing — `wrangler rollback` is a bounded action, untested rollback is worse than no rollback). Gate (b) is now the only blocker. Roadmap Phase D.4. See `docs/strategy-five-pillars-2026-05-02.md`.
- **Reversibility audit doc (pillar 4)**: One-page `docs/reversibility-audit.md` enumerating strategic CF lock-in vs commodity dependencies; cost-of-departure ledger in engineer-weeks. Roadmap Phase C.7. See `docs/strategy-five-pillars-2026-05-02.md`.
...
- 13 CI/CD workflows, 7 R2 buckets, 5 KV namespaces, 2 Durable Objects, 2 Analytics Engine datasets, **1 Hyperdrive binding** (`pitchey-db-real`, caching disabled, auth-path only)
- Pillar 1 acceptance gate (a) MET: untagged `.catch(() => …)` residue across `src/` = 0 (was 196 pre-remediation)
...
- **Pillar 1 verification disambiguated**: orchestrator gate is two-count discipline — untagged `.catch(() => …)` residue < 30 (excluding `worker-integrated.ts`); telemetry sites must be marked `// fire-and-forget` on the prior line to be excluded from the count. Forces a per-site human judgment at each remaining catch.
...
### Session 2026-05-08 — shipped

Bundled into squash commit `cf5394f5` via PR #106 — three concerns merged together, all already running in prod after two same-session deploys (worker versions `4619c6e2…` then `b171af87…`). Retrospective PR brought `main` back in sync with prod.

- **Hyperdrive on auth path** — session lookups previously bypassed Hyperdrive (which wasn't even bound; defensive `env.HYPERDRIVE?.connectionString || env.DATABASE_URL` fallback in `db/worker-client.ts:27` always fell through). Wired `session-store.ts` + `legacy-session-handler.ts` to use the same fallback pattern. Existing CF resource `pitchey-db-real` (`dec8d1ab675649c8b69d88cb4c160704`) updated with current Neon URL + caching explicitly disabled (`--caching-disabled`). Orphan duplicate `pitchey-db` (`bc2d846d32e64b5c8122654e4f12c959`) deleted. Caching choice is security-relevant: query result caching on session lookups would let revoked sessions keep authenticating until cache TTL expires. wrangler.toml has a comment block on the rotation gotcha — Hyperdrive holds its own credential copy, must `wrangler hyperdrive update --connection-string=...` AND `wrangler secret put DATABASE_URL` together when Neon password rotates.
- **Cron `cleanupDatabase` placeholder fixed** — was a `console.log("Cleaning up database...")` stub. Cron fired every Mon 2am and did nothing. Expired session rows accumulated indefinitely except via opportunistic logout-path deletion (which most users never trigger; ~70-85% of sessions become orphan rows after 30-day expiry). Body now mirrors `updateTrendingAlgorithm` pattern: structured JSON log (`category=session_cleanup`, includes `deleted` count + `duration_ms` — picked up by Axiom via 100% sampling), Sentry forward on failure under `cron: cleanup_database` tag. `deleteExpiredSessions()` return type changed `Promise<void>` → `Promise<number>` so the log can report the count.
- **#97 closed** — `creator-dashboard-extended.ts:36` had `(${months} || ' months')::interval` which fails on parameterized integer (Postgres rejects `integer || unknown`). Replaced with `(INTERVAL '1 month' * ${months})` — standard PG idiom. Pre-existing bug surfaced after PR #102 wrapped the surrounding query in `safeQuery`; would have started reporting `creator-dashboard.revenue.trends` to Sentry on first authenticated creator request. Lands before the Tue 2026-05-13 Sentry-watch window so a known bug doesn't trip the gate.

#### Discoveries this session worth tracking

- **#103 filed:** 7 of 8 scheduled cron handlers in `worker-integrated.ts` are placeholder stubs that just `console.log` and return — `checkNDAExpirations`, `checkContainerHealth`, `monitorJobQueues`, `sendDigestEmails`, `aggregateMetrics`, `exportAuditLogs`, `archiveOldJobs`. Cron triggers fire on schedule; nothing happens. Each represents a feature the platform appears to have but doesn't (e.g. "audit logs are exported daily" → no). Audit-and-decide work tracked.
- **#104 filed:** Frontend orphan handler tree pattern, like worker Era 0-6 orphans but on the frontend. Canonical example: `frontend/src/services/auth.service.ts` extensively uses BA-react's `authClient` but is imported only by its own test file. Naive grep over-counts because of React.lazy lazy-loaded routes — real audit needs `knip`/`ts-prune`/`madge`.
- **#105 filed:** BA Phase 1.5a-frontend with corrected scope. Earlier framing assumed "BA was ripped" applied platform-wide; in reality the rip was backend-only. Frontend bundles `better-auth` (~100-200KB) but `createAuthClient` is dead-loaded — the resulting `authClient` is never consumed for HTTP (live frontend auth uses plain fetch wrappers). Rip is bounded (delete `auth.service.ts` + tests, strip `createAuthClient` from `better-auth-client.tsx`, `npm uninstall`, frontend Pages deploy). Realistic 30-45 min, deferred post-Stripe.

#### #20 near-miss from 2026-05-08

- **"BA Phase 1.5a is a 5-min `npm uninstall`"** → almost ran `npm uninstall better-auth` against the live frontend, which would have broken the build because `auth.service.ts` references `authClient.*` methods. Frontend BA situation was undocumented; CLAUDE.md "Better Auth was ripped 2026-05-04" framing was inherited unqualified into the task list. Caught by pre-flight grep that found live-looking imports in `frontend/src/`. Fix: investigation revealed the dead-loaded vs orphan-consumer distinction; memory + this CLAUDE.md now reflect the corrected state, work tracked in #105.

#### Stale issues closed

- **#80, #81, #75, #66** — catch-swallow tracking issues from earlier phases, all now resolved by the merged Phase 1b/2.x PRs (closed during this session with resolution comments referencing the merged PRs).

#### Hyperdrive permission learning

`CLOUDFLARE_API_TOKEN` (the "Edit Cloudflare Workers" template) didn't include Hyperdrive scope — Hyperdrive is newer than that template. User had to add `Account → Hyperdrive → Edit` to the existing token before `wrangler hyperdrive create/update/list` worked. Worth noting for future token audits — the template-based tokens drift behind newer CF products.
