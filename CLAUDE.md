...
- **Auth**: Custom sessions only (Better Auth ripped 2026-05-04 per issue #19, path a). Legacy `users`/`sessions` tables, `pitchey-session` UUID cookie, custom login/logout/refresh handlers inlined in `src/worker-integrated.ts`. The session-store helper lives at `src/auth/session-store.ts` (4 raw-SQL methods used by the inline login paths). The auth-adapter at `src/auth/auth-adapter.ts` validates session cookies + JWT bearer tokens for `/api/users/profile` + `/api/users/settings`. WebSocket auth uses `src/auth/legacy-session-handler.ts`. The `better-auth` + `better-auth-cloudflare` deps were **uninstalled 2026-06-05** (reversibility window long passed; verified nothing live imported them — the only frontend reference was a dead-loaded `createAuthClient` in `better-auth-client.tsx`, removed). Cleared the recurring `better-auth` npm-audit CI failure and shrank the frontend bundle. The remaining `better-auth-*`-named files are legacy-named plain-fetch/raw-SQL shims, not the BA library. Still TODO: drop the empty BA tables (`user`, `session`, `account`, `verification`). Why path (a) over (b): the 2026-05-02 verdict picked migration to BA, but re-reading the live code showed BA was never actually authenticating anyone — `dbAdapter` was a misnamed raw-SQL shim, BA library calls were guarded fallbacks behind legacy lookups. Same shape as the #20 pattern: reasoning from doc framing rather than runtime. Migration would have meant *building* a BA flow from scratch, not migrating between two working systems.
- **Cache**: Upstash Redis (global)
- **Storage**: Cloudflare R2
- **Reversibility**: Strategic CF lock-in (proprietary, expensive to leave) — Durable Objects, Analytics Engine, Hyperdrive, R2, Workers AI (when adopted). Commodity / portable — Hono, raw SQL via `@neondatabase/serverless`, Zod, React. Cost-of-departure ledger in `docs/reversibility-audit.md` (~12–17 engineer-weeks, dominated by DO + AE).
...
- **`www.pitchey.com` is the canonical user-facing URL** (domain cutover 2026-07-01): `pitchey.com` + `www.pitchey.com` are custom domains on the live `pitchey` Pages project; the apex 301s to www; the old `pitchey-coming-soon` worker routes (`pitchey.com/*`, `www.pitchey.com/*` on the same account's zone) were deleted (backup of route ids in the 2026-07-01 session scratchpad). `FRONTEND_URL` in `wrangler.toml` = `https://www.pitchey.com`.
- **`pitchey-5o8.pages.dev` is the underlying Pages hostname — keep it.** Same project, same deployment as www.pitchey.com; CI health checks, smoke tests, and Lighthouse target it, and preview deploys live at `*.pitchey-5o8.pages.dev`. It cannot be detached from a Pages project anyway. The `-5o8` suffix is **permanent** (Cloudflare assigns it for the project's lifetime; it is not a preview/orphan). Verify live via the bundle hash `curl -s https://pitchey-5o8.pages.dev/ | grep -oE 'index-[A-Za-z0-9_]+\.js'`.
...
- Raw SQL only (no ORM)
- Sessions live in the legacy `sessions` table; cookie is `pitchey-session` (UUID). JWT bearer tokens are accepted as a fallback in `auth-adapter.ts:validateAuth`. Better Auth was ripped 2026-05-04 per issue #19; the `better-auth`/`better-auth-cloudflare` deps were uninstalled 2026-06-05 (nothing live imported them).
...
- **Heat Score Algorithm**: Bayesian + role-weighted scoring — PG function `recalculate_heat_scores()` (kept fresh by the live `*/15` cron → `updateTrendingAlgorithm`), `heat_role_weights` config table, 3 API routes (`/pitches/hot`, `/pitches/:id/heat`, admin recalc). heat badges (Hot/Trending). **`heat_score` powers `/api/pitches/hot` + `/api/pitches/trending` (`getTrending`) only.** The **marketplace browse path the frontend actually calls — `/api/pitches/browse` (`browsePitches`) — does NOT use heat_score**: its `trending` and `popular` tabs rank by `COALESCE(view_count,0) + COALESCE(like_count,0)*3 DESC`, `new` by recency. (Earlier doc said "Trending endpoint now sorts by heat_score" — true for `/trending`, **false for the browse handler `MarketplaceEnhanced` calls**; corrected 2026-06-30.)
- **"R15" (2026-06-30 roadmap: retire `view_count` ranking for `heat_score` in marketplace) is DEFERRED post-launch — empirically, not a phantom.** A prod distribution check (2026-06-30, 12 published pitches) showed `heat_score` is **recency-dominated at low N**, not engagement-dominated: it ranked the newest/lowest-engagement test pitches ("Bob", 1 view) **above** the most-viewed real pitch ("At Ever Las", 14 views) — i.e. inversely correlated with engagement. Mechanism: `rating_score = 0.3·bayesian_rating·10` collapses to a **constant ~18 prior** with no real ratings (zero signal), `engagement_score` is tiny at single-digit views, so `freshness_score` (recency) is the only term that varies → ordering ≈ recency. Switching the browse `trending` tab to `heat_score` now would surface test/newest pitches to the top of the marketplace — strictly worse than the current `view_count+likes`. heat becomes the correct trending signal once real rating/engagement volume accumulates; **revisit R15 post-launch, don't ship blind.** Tuning the rating-prior/freshness weighting so heat is usable at low N is a separate heat-algorithm effort, not R15.
...
- **URL Consolidation — corrected model** (2026-04-18 → 2026-04-21, finalized 2026-05-02): PRs #14 + #21 swept hostname references from `pitchey-5o8.pages.dev` to `pitchey.pages.dev` under an incorrect "same project, two subdomains" mental model. In reality, these were **two separate Pages projects on two separate CF accounts**, both literally named `pitchey` — CF suffixes `-5o8` when the global `<name>.pages.dev` slot is already claimed. Cavelltheleaddev's `pitchey` project (stale December 2025 canonical) was **deleted 2026-04-21**: 402 deployments purged + project DELETE'd via CF API. `pitchey.pages.dev` now NXDOMAINs. Ndlovucavelle's `pitchey` project, served at `pitchey-5o8.pages.dev` (account ID `002bd5c0e90ae753a387c60546cf6869`), is the actual live frontend. `pitchey.com` is a `pitchey-coming-soon` marketing stub on a separate Pages project — not the real frontend. `FRONTEND_URL` in `wrangler.toml:159` still stale (points at the dead subdomain); needs a Worker redeploy with the correct value. See `docs/sessions/2026-04-21-URGENT-status.md` for the full timeline. **Subdomain suffix is permanent** (verified 2026-05-02 via CF API while resolving #18): the `-5o8` suffix sticks for the project's lifetime — CF Pages does NOT retroactively rename project subdomains when a previously-claimed global slot frees up. So `pitchey-5o8.pages.dev` IS the canonical production URL — there is **no orphan project** to clean up, and there is no path to "reclaim" `pitchey.pages.dev` short of creating a new project + cutover. If the `-5o8` suffix becomes a branding concern, the fix is a custom domain (e.g. `app.pitchey.com`), not a project rename. **Update 2026-07-01: that custom-domain cutover happened** — `pitchey.com`/`www.pitchey.com` now serve the app from the `pitchey` Pages project (www canonical, apex 301s), the coming-soon worker routes are deleted, and `FRONTEND_URL` is fixed to `https://www.pitchey.com`. The "marketing stub" wording earlier in this entry is historical.
...
- **Per-Portal Profile Settings + Branded Slate Distribution** (2026-06-12, commit `814947b2`, live on canonical): role-tailored profile pages at `/<portal>/settings/profile` — Creator (creative statement + portfolio) and Investor (fund name + investment thesis + NDA address) are new; Production's orphaned `ProductionSettingsProfile` wired up + its save patched with the #280 session-refresh. Investor settings previously had **no username/identity field at all**. Shared `Settings.tsx` Profile tab gained a portal-aware launcher (creator/production). `ProfileUpdateInput` extended (firstName/lastName/companyName/companyAddress/profileImage — backend already accepted them). Slate editor (`CreatorSlateDetail`) gained a real cover/banner uploader (reuses `/api/upload/profile` folder=`covers`, free; `cover` 1920px preset; persisted via `SlateService.update`) — renders as the hero on the public `/slates/s/:id`. **Two business workflows unlocked** (documented in `docs/ui-business-audit.md` flows 4–5): targeted outreach via published identity/thesis, and branded slate distribution as the platform's main outbound sharing surface. Note: the cover control in `ProductionSettingsProfile` remains a dead/non-functional stub (local base64, never persisted); the slate is the only real banner.

### TODO
- **Unit-economics dashboard (pillar 2)**: 1–2 North Star unit metrics in Analytics Engine — cost per pitch view leading candidate, cost per NDA signature / per investor onboarded / per Heat recomputation as alternates. Backed by SQL over the surviving 2 AE datasets; do not add new bindings. Roadmap Phase C.6. See `docs/strategy-five-pillars-2026-05-02.md`.
- **Orchestrator agent — AIOps (pillar 3)**: Long-running agent consuming Sentry issue webhooks + Axiom alert routes; bounded remediations via Cloudflare + Sentry MCP (rollback, restart Durable Object, flush Hyperdrive cache). **Hard prerequisites (both required)**: (a) catch-swallow sweep complete — **DONE 2026-06-16**: the gate now runs `--include-worker --threshold 0`, so *zero* untagged `.catch(() => …)` across all of `src/` **including** `worker-integrated.ts` (the old "< 30 excluding worker-integrated.ts" bar is superseded; the safe `request.json().catch(() => ({}))` body-parse idiom is exempt in the gate itself); (b) Phase C.1 rollback drill verified end-to-end against staging — `wrangler rollback` is a bounded action, untested rollback is worse than no rollback. **Both prerequisites now MET** (catch-swallow at 0 untagged; rollback drill documented executed in `docs/ROLLBACK_DRILL_RUNBOOK.md`) — pillar 3 is unblocked, not started. Roadmap Phase D.4. See `docs/strategy-five-pillars-2026-05-02.md`.
- **Reversibility audit doc (pillar 4)**: ✅ DONE — `docs/reversibility-audit.md` exists (cost-of-departure ledger, ~12–17 engineer-weeks). (Was a stale TODO; doc shipped 2026-05-06.)
...
### Session 2026-05-02 — shipped

- **Strategic frame doc** (`docs/strategy-five-pillars-2026-05-02.md`, 116 lines) capturing the five-pillar translation of hyperscaler research: catch-swallow remediation → unit economics → orchestrator agent → reversibility audit. Workers AI deferred until a real ML feature lands.
- **Roadmap cross-linked**: Strategic frame callout in Context; Phase C.3 annotated as pillar 1 / orchestrator prerequisite; new Phase C.6 (unit economics, pillar 2), Phase C.7 (reversibility audit, pillar 4), Phase D.4 (orchestrator agent, pillar 3).
- **CLAUDE.md**: Architecture reversibility line added (strategic CF lock-in vs commodity); three TODO bullets for pillars 2/3/4 with roadmap and strategy-doc cross-references.
- **Pillar 1 verification disambiguated**: orchestrator gate is two-count discipline — untagged `.catch(() => …)` residue < 30 (excluding `worker-integrated.ts`); telemetry sites must be marked `// fire-and-forget` on the prior line to be excluded from the count. Forces a per-site human judgment at each remaining catch. **Update 2026-06-16**: bar tightened to **0 untagged including `worker-integrated.ts`** (`catch-swallow-gate.mjs --include-worker --threshold 0`, wired into `ci-cd.yml`). Caught 4 laundered tags in the router — user mutations (delete scheduled report, verification request, company-verify submit, demo request) tagged `// fire-and-forget` but returning success on a swallowed write; all fixed to propagate to the handler `try/catch`. Lesson: a "fire-and-forget" tag on anything the user is told succeeded is laundering — same #20 shape.
- **Phase D.4 gates both required**: catch-swallow + Phase C.1 rollback drill verified end-to-end. CLAUDE.md TODO and roadmap entry both name both prerequisites — an orchestrator that can call an untested `wrangler rollback` is worse than no orchestrator.
- **No code.** Frame-only session — execution belongs to the phases referenced.

### Session 2026-05-27 — shipped: Stripe LIVE

**Stripe is live in production** (branch `docs/coverage-notes-scope-and-next-actions`, commits `59d73b4d`..`5606f10d`, worker version `e3f533e8`, **not yet merged to main**).

- **4 new webhook handlers added** to `handleStripeWebhook` (commit `59d73b4d`): `customer.subscription.created` (defensive, dedupes vs checkout flow), `invoice.payment_failed` (writes failure detail to `subscription_history.metadata`, keeps status active through dunning), `charge.refunded` (log-only at error level — credit_transactions has no `payment_intent_id` linkage), `charge.dispute.created` (log-only at error level for ops alerting). All four ride the existing `stripe_webhook_events` idempotency gate.
- **`users.stripe_customer_id` column** (migration 088, nullable + partial index). Populated by `checkout.session.completed` from `session.customer`.
- **Customer Portal wiring**: `POST /api/payments/billing-portal` endpoint (resolves customer_id with `getOrCreateCustomer` email-search fallback), shared `StripePortalCard` component mounted on `Billing.tsx` (creator/investor/production) + `ProductionSettingsBilling.tsx`. Watcher excluded.
- **Webhook auth fix** (commit `92aa3cde`): `/api/webhooks/stripe` added to `publicEndpoints` — previously hit `validateAuth` and returned 401 in 0ms before reaching the handler. Security model is HMAC-SHA256 inside the handler.
- **Stripe API v2025 field drift** (commit `9dddebea`): `sub.current_period_*` moved to `sub.items.data[0]`, `invoice.subscription` moved to `invoice.parent.subscription_details.subscription`. Both fired silent failures in test mode (handlers errored, returned 200 to Stripe, DB state incomplete). Fixed at 4 sites; pattern memo in [[feedback_stripe_api_v2_drift]].
- **`SubscriptionCard` rewrite** (commit `7023dcef`): the component had hardcoded basic/pro/enterprise tiers at $29/$99/$299 that pre-dated the Creator/Production/Exec ladder — every "Upgrade to Basic" sent `tier=basic` which getSubscriptionTier returned null for, surfacing as "No checkout URL received." Now reads `getSubscriptionTiersByUserType(userType)` from config, uses tier.id as planKey, displays €X.XX prices. Dropped dead "Manage in Stripe" link to generic billing.stripe.com (StripePortalCard handles this now).
- **Live mode setup**: `scripts/stripe-go-live.sh` (commit `5606f10d`) one-shot — prompts for sk_live no-echo, creates webhook + portal config + sets secrets + deploys. Live IDs: webhook `we_1TbpEAGfa7gtG8QyNz0mMdC8`, default Customer Portal config `bpc_1TbpEBGfa7gtG8QyubavJQ2p`.
- **Price IDs swapped to live** (commit `5606f10d`): 16 price IDs in `subscription-plans.ts` swapped from test-mode → live-mode 1:1 by euro amount. Single-mode config — flipping back to test requires reverting that commit (or a future env-aware tier map).
...
**Production Frontend**: `https://www.pitchey.com` (canonical since 2026-07-01; apex 301s to www) — same deployment as `https://pitchey-5o8.pages.dev` (underlying Pages hostname, CI target). The coming-soon page is gone.
...
- **Disaster recovery**: `docs/DISASTER_RECOVERY.md` (catastrophic loss — Neon PITR/branch restore, rebuild-from-zero, infra inventory). Honest about gaps: secrets have NO backup (write-only), R2 has NO versioning, and the local `scripts/*backup*.sh` are unverified/partly-broken (like the rollback script was).
...
- **`ndas.expires_at` / `nda_requests.expires_at` are intentionally DEAD — do NOT enforce or "wire up."** Commit `0a92edb0` (2026-04-10) removed NDA expiry as a deliberate product decision (Karl): **NDAs no longer expire (NDA permanence)**. The columns + their indexes (`idx_ndas_expires`, `idx_pitch_access_expires`, the composite `idx_ndas_signed`) still exist and ~12+ live sites still `SELECT` the column and pass it through as `expiresAt` (always NULL) in API responses — but the live access gate (`worker-integrated.ts` `ndaCheck`: `status IN ('approved','signed')`, no `expires_at` filter) does **not** enforce it, and `requestNDA`'s INSERT does **not** write it. The only expiry *logic* (`nda-workflow.service.ts` `expires_at < NOW()`, `scheduleExpirationCheck`) is imported solely by the parked `src/workflows/` worker (PROTECTED, #60) — dead in prod. **An audit will keep flagging "NDAs don't expire = security gap"; that is a phantom (#20 shape) — the gap is intentional.** Adding expiry enforcement reverses a locked decision and needs Karl, not a code task. The column was kept (not dropped) on purpose: dropping it means editing those ~12+ live SELECT/response sites in the thinly-tested 22.5k-line worker — high-effort, low-value, regression-prone for an always-NULL nullable column. ("R14" in the 2026-06-30 evolution roadmap was this phantom; closed as no-op.) Separately, the per-request **10-credit charge + per-pitch dedupe** (`ALREADY_EXISTS` before the charge) already throttle request spam — no NDA-request rate-limiter is needed, and one would double-gate against the Karl-locked credit model.
