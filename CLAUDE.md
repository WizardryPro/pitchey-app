...
- **Auth**: Custom sessions only (Better Auth ripped 2026-05-04 per issue #19, path a). Legacy `users`/`sessions` tables, `pitchey-session` UUID cookie, custom login/logout/refresh handlers inlined in `src/worker-integrated.ts`. The session-store helper lives at `src/auth/session-store.ts` (4 raw-SQL methods used by the inline login paths). The auth-adapter at `src/auth/auth-adapter.ts` validates session cookies + JWT bearer tokens for `/api/users/profile` + `/api/users/settings`. WebSocket auth uses `src/auth/legacy-session-handler.ts`. The `better-auth` and `better-auth-cloudflare` deps remain in `package.json` for a 1-week reversibility window; Phase 1.5 will uninstall and drop the empty BA tables (`user`, `session`, `account`, `verification`). Why path (a) over (b): the 2026-05-02 verdict picked migration to BA, but re-reading the live code showed BA was never actually authenticating anyone — `dbAdapter` was a misnamed raw-SQL shim, BA library calls were guarded fallbacks behind legacy lookups. Same shape as the #20 pattern: reasoning from doc framing rather than runtime. Migration would have meant *building* a BA flow from scratch, not migrating between two working systems.
...
- Raw SQL only (no ORM)
- Sessions live in the legacy `sessions` table; cookie is `pitchey-session` (UUID). JWT bearer tokens are accepted as a fallback in `auth-adapter.ts:validateAuth`. Better Auth was ripped 2026-05-04 per issue #19; deps remain in `package.json` for the Phase 1.5 reversibility window.
