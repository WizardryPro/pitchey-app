...
- **Orchestrator agent — AIOps (pillar 3)**: Long-running agent consuming Sentry issue webhooks + Axiom alert routes; bounded remediations via Cloudflare + Sentry MCP (rollback, restart Durable Object, flush Hyperdrive cache). **Hard prerequisites (both required)**: (a) catch-swallow sweep complete — **DONE 2026-06-16**: the gate now runs `--include-worker --threshold 0`, so *zero* untagged `.catch(() => …)` across all of `src/` **including** `worker-integrated.ts` (the old "< 30 excluding worker-integrated.ts" bar is superseded; the safe `request.json().catch(() => ({}))` body-parse idiom is exempt in the gate itself); (b) Phase C.1 rollback drill verified end-to-end against staging — `wrangler rollback` is a bounded action, untested rollback is worse than no rollback. Roadmap Phase D.4. See `docs/strategy-five-pillars-2026-05-02.md`.
