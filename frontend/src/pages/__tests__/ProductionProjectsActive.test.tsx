...
beforeAll(async () => {
  const mod = await import('@portals/production/pages/ProductionProjectsActive')
...
describe('ProductionProjectsActive', () => {
  ...
  it('fetches from correct API endpoint with active status', async () => {
    mockFetch.mockResolvedValue({
      ok: true,
      json: () => Promise.resolve({ projects: [] }),
    })
    ...
    expect(calledUrl).toContain('stage=production')
  })
})
